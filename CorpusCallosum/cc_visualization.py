...
def options_parse() -> argparse.Namespace:
    ...
    parser.add_argument("--measurement_points", type=str, required=True,
    ...
    parser.add_argument("--smooth_iterations", type=int, default=1, help="Number of smoothing iterations to apply to the mesh")
    parser.add_argument("--colormap", type=str, default="red_to_yellow", 
                       choices=["red_to_blue", "blue_to_red", "red_to_yellow", "yellow_to_red"],
                       help="Colormap to use for thickness visualization")
    parser.add_argument("--color_range", type=float, nargs=2, default=None,
                       metavar=('MIN', 'MAX'),
...
def main(contours_path: str | Path | None, thickness_path: str | Path, measurement_points_path: str | Path,
         output_dir: str | Path, resolution: float = 1.0, smooth_iterations: int = 1, 
         colormap: str = "red_to_yellow", color_range: tuple[float, float] | None = None,
         legend: str | None = None, twoD: bool = False) -> None:
    ...
    if twoD:
        ...
        cc_mesh.plot_cc_contour_with_levelsets(contour_idx=0, levelpaths=None, title=None, save_path=str(output_dir / 'cc_thickness_2d.png'), colorbar=True)
    else:
        ...
        cc_mesh.plot_mesh(colormap=colormap, color_range=color_range, thickness_overlay=True, show_contours=False, show_mesh_edges=True, legend=legend)
        cc_mesh.plot_mesh(str(output_dir / 'cc_mesh.html'), thickness_overlay=True)

        cc_mesh.plot_cc_contour_with_levelsets(contour_idx=len(cc_mesh.contours)//2, save_path=str(output_dir / 'midslice_2d.png'))
