...
def get_mapping_to_standard_space(orig, ac_coords_3d, pc_coords_3d, orig_fsaverage_vox2vox, output_dir):
    """
    ...
    nod_correct_3d[1:3,3] = nod_correct_2d[:2,2]     # Copy translation part to y,z axes (usually no translation)

    
    ac_coords_after_nodding = apply_transform_to_pt(ac_coords_3d, nod_correct_3d, inv=False)
    pc_coords_after_nodding = apply_transform_to_pt(pc_coords_3d, nod_correct_3d, inv=False)
    ...
    # correct nodding
    ac_coords_standardized = apply_transform_to_pt(ac_coords_after_nodding, ac_to_center_translation, inv=False)
    pc_coords_standardized = apply_transform_to_pt(pc_coords_after_nodding, ac_to_center_translation, inv=False)

    standardized_to_orig_vox2vox = np.linalg.inv(orig_fsaverage_vox2vox) @ np.linalg.inv(nod_correct_3d) @ np.linalg.inv(ac_to_center_translation)
    
    # calculate ac & pc in space of mri input image
    ac_coords_orig = apply_transform_to_pt(ac_coords_standardized, standardized_to_orig_vox2vox, inv=False)
    pc_coords_orig = apply_transform_to_pt(pc_coords_standardized, standardized_to_orig_vox2vox, inv=False)

    return standardized_to_orig_vox2vox, ac_coords_standardized, pc_coords_standardized, ac_coords_orig, pc_coords_orig


def apply_transform_and_map_volume(volume, transform, affine, header, output_path=None, order=3, output_size=None):
    """
    ...
    transformed = affine_transform(volume.astype(np.float32), np.linalg.inv(transform), output_shape=output_size, order=order)
...
def map_softlabels_to_orig(outputs_soft, orig_fsaverage_vox2vox, orig, slices_to_analyze, orig_space_segmentation_path = None, fsaverage_middle=128):
    """
    ...
    for i in range(outputs_soft.shape[-1]):
        ...
        outputs_soft_padded[fsaverage_middle-slices_to_analyze//2:fsaverage_middle+slices_to_analyze//2+1] = outputs_soft[...,i]
    ...
    # nib.save(nib.MGHImage(outputs_soft, seg_affine, transformed_img.header), Path(output_dir) / "softlabels_seg_space.mgz")
    # nib.save(nib.MGHImage(softlabels_orig_space, orig.affine, orig.header), Path(output_dir) / "softlabels_orig_space.mgz")

    # apply softmax to softlabels_orig_space
    softlabels_orig_space = np.exp(softlabels_orig_space) / np.sum(np.exp(softlabels_orig_space), axis=-1, keepdims=True)

    segmentation_orig_space = np.argmax(softlabels_orig_space, axis=-1)
    segmentation_orig_space = np.where(segmentation_orig_space == 1, 192, segmentation_orig_space)
    segmentation_orig_space = np.where(segmentation_orig_space == 2, 250, segmentation_orig_space)

    if orig_space_segmentation_path is not None:
        nib.save(nib.MGHImage(segmentation_orig_space, orig.affine, orig.header), orig_space_segmentation_path)
...
def interpolate_midplane(orig, orig_fsaverage_vox2vox, slices_to_analyze):
    ...
    # make grid of 9 slices in the fsaverage middle (cube from 123.5,0.5,0.5 to 132.5,255.5,255.5 (incudling end points, 1mm spacing))
    x_coords = np.linspace(124-slices_to_analyze//2, 132+slices_to_analyze//2, 9+(slices_to_analyze-1), endpoint=True)  # 9 points from 123.5 to 132.5
    #x_coords = np.linspace(orig.shape[0]//2-slice_thickness//2, orig.shape[0]//2+slice_thickness//2, slice_thickness, endpoint=True)
    y_coords = np.linspace(0, orig.shape[1]-1, orig.shape[1], endpoint=True)  # 255 points from 0.5 to 255.5 
    z_coords = np.linspace(0, orig.shape[2]-1, orig.shape[2], endpoint=True)  # 255 points from 0.5 to 255.5
    X, Y, Z = np.meshgrid(x_coords, y_coords, z_coords, indexing='ij')
    
    # Stack coordinates and add homogeneous coordinate
    grid_fsaverage = np.stack([
        X.ravel(),
        Y.ravel(), 
        Z.ravel(),
        np.ones(X.size)
