...
#
# Deliberately not listed: stat, date, readlink, grep and sed. Each has GNU-only options too, but a
# guarded GNU branch is a perfectly good way to use them, as recon_surf/functions.sh does by probing
# `stat --version` first, and a textual check cannot tell that from an unguarded one.
GNU_ONLY = {
