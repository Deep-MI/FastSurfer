...
def transform_axial(vol, coronal2axial=True):
    """
    Function to transform volume into Axial axis and back (RAS [coronal] to ASR [axial])
...
def transform_sagittal(vol, coronal2sagittal=True):
    """
    Function to transform volume into Sagittal axis and back (RAS [coronal] to SAR [saggital])
    :param np.ndarray vol: image volume to transform
...
def get_thick_slices(img_data, slice_thickness : int = 3, pad : bool = True):
    """
    ...
    h, w, d = img_data_pad.shape
    img_data_pad = np.expand_dims(img_data_pad, axis=3)
    img_data_thick = np.ndarray((h, w, 2*slice_thickness+1, 0), dtype=np.uint8)

    for slice_idx in range(slice_thickness, d-slice_thickness):
        img_data_thick = np.append(img_data_thick, img_data_pad[:, :, slice_idx-slice_thickness:slice_idx+slice_thickness+1, :], axis=3)

    # (plane1, plane2, slice_thickness, no_thick_slices) ->  (plane1, plane2, no_thick_slices, slice_thickness)
    img_data_thick = np.transpose(img_data_thick, (0, 1, 3, 2))

    return img_data_thick


def filter_blank_slices_thick(volume_list: list, label_vol: np.ndarray, threshold=50):
    """
    ...
    select_slices = (np.sum((label_vol != 0)*1, axis=(0, 1)) > threshold)
...
def create_weight_mask(mapped_aseg, max_weight=5, max_edge_weight=5, max_hires_weight=None, ctx_thresh=33,
    ...
    """
    labels, counts = np.unique(mapped_aseg, return_counts=True)

    for indx, label in enumerate(labels):
        mapped_aseg[mapped_aseg==label] = indx
    ...
    if max_hires_weight is not None:
        # High-res Weighting
        LOGGER.info(f"Adding hires weight mask deep sulci and WM with weight {max_hires_weight}")
        ...
        if cortex_mask:
            LOGGER.info(f"Adding cortex mask with weight {max_hires_weight}")
...
def deep_sulci_and_wm_strand_mask(volume, structure, iteration=1, ctx_thresh=33):
    ...
    LOGGER.info(f"Remaining voxels sulci/wm strand: {np.unique(diff_image, return_counts=True)}")
    return diff_image
...
def split_cortex_labels(aparc):
    ...
    aseg_lh = filters.gaussian_filter(1000 * np.asarray(aparc == 2, dtype=np.float32), sigma=3)
    aseg_rh = filters.gaussian_filter(1000 * np.asarray(aparc == 41, dtype=np.float32), sigma=3)
...
def unify_lateralized_labels(lut, combi=("Left-", "Right-")):
    ...
    #left["LabelName"] = left["LabelName"].str.removeprefix(combi[0])  # only for python3.9 and above
    #right["LabelName"] = right["LabelName"].str.removeprefix(combi[1])
    left["LabelName"] = left["LabelName"].str.replace(combi[0], "")
    right["LabelName"] = right["LabelName"].str.replace(combi[1], "")
...
def map_aparc_aseg2label(aseg, labels, labels_sag, sagittal_lut_dict, aseg_nocc=None, processing="aparc"):
    ...
    h, w, d = aseg.shape
    lut_aseg = np.zeros(max(labels) + 1, dtype='uint16')
    ...
    h, w, d = aseg.shape
    lut_aseg = np.zeros(max(labels_sag) + 1, dtype='uint16')
...
def map_prediction_sagittal2full(prediction_sag, num_classes=51, lut=None):
    ...
    if num_classes == 96:
        ...
        idx_list = np.asarray([0, 1, 2], dtype=np.int16)
    elif num_classes == 2:
...
def subset_volume_plane(volume: np.ndarray, plane: Union[int, float] = 128, thickness: int = 5, axis: int = 0):
        """Return a subset of the volume around the plane

        Args:
            volume (np.ndarray): volume to subset
            plane (Union[int, float], optional): plane of the subset. Defaults to 127.5.
            thickness (int, optional): Total thickness of the subset. Defaults to 7. If odd, the plane should be inbetween two indices.
            axis (int, optional): axis of the plane. Defaults to 0.
...
def pad_to_size(volume: np.ndarray, size: tuple, plane: Union[int, float] = 128, axis: int = 0):
    ...
    Args:
        volume (np.ndarray): volume to pad
        size (int): size to pad to
        plane (Union[int, float], optional): plane of the subset. Defaults to 128.
        axis (int, optional): axis of the plane. Defaults to 0.
