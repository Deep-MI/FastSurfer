...
# from CCNet.utils.arg_types import (vox_size as __vox_size, target_dtype as __target_dtype,
...
def options_parse():
    ...
    parser.add_argument('--vox_size', dest='vox_size', default=1.,
    ...
    advanced.add_argument('--conform_to_1mm_threshold',
...
def map_image(img: nib.analyze.SpatialImage, out_affine: np.ndarray, out_shape: np.ndarray,
              ras2ras = None,
              order: int = 1, dtype = None) -> np.ndarray:
    """
...
def getscale(data: np.ndarray, dst_min: float, dst_max: float,
             f_low: float = 0.0, f_high: float = 0.999):
    """
...
def conform(img: nib.analyze.SpatialImage,
            order: int = 1, conform_vox_size = 1., dtype = None,
            conform_to_1mm_threshold = None):
    f"""
...
def is_conform(img: nib.analyze.SpatialImage,
               conform_vox_size = 1., eps: float = 1e-06, check_dtype: bool = True,
               dtype = None, verbose: bool = True,
               conform_to_1mm_threshold = None) -> bool:
    f"""
...
def get_conformed_vox_img_size(img: nib.analyze.SpatialImage, conform_vox_size,
                               conform_to_1mm_threshold = None):
    """Extract the voxel size and the image size."""
    # this is similar to mri_convert --conform_min
    try:
        conform_vox_size = float(conform_vox_size)
    except:
        pass
    ...
    try:
        ...
        elif 0. < conform_vox_size <= 1.0:
            conformed_vox_size = conform_vox_size
    except:
        print(conform_vox_size)
        raise ValueError(f"Invalid value for conform_vox_size passed: {conform_vox_size}")
    conformed_img_size = find_img_size_by_fov(img, conformed_vox_size)
    return conformed_vox_size, conformed_img_size


def check_affine_in_nifti(img, logger = None) -> bool:
    """
