...
class RunModelOnData:
    ...
    def __init__(self, args):
        ...
        self.does_inpainting = any(view["cfg"].MODEL.MODEL_NAME == 'FastSurferPaint' for view in self.view_ops.values())

        self.does_localisation = {view : self.view_ops[view]["cfg"].MODEL.MODEL_NAME == 'FastSurferLocalisation' for view in self.view_ops}

        self.models = {}
        for plane, view in self.view_ops.items():
            if view["cfg"] is not None and view["ckpt"] is not None:
                self.models[plane] = Inference(view["cfg"], ckpt=view["ckpt"], device=device, inference_weights={"sagittal": args.inferece_weights[0], "coronal": args.inferece_weights[1], "axial": args.inferece_weights[2]})
    ...
    def get_prediction(self, orig_f: str, orig_data: np.ndarray, zoom: Union[np.ndarray, tuple], lesion_mask=None, crop : bool = False, extra_slices : int = 0) -> np.ndarray:
        
        if not crop:
            shape = orig_data.shape + (self.get_num_classes(),)
            shape_loc = orig_data.shape
        else:
            shape = (orig_data.shape[0]-extra_slices, orig_data.shape[1], orig_data.shape[2],self.get_num_classes())
            shape_loc = (orig_data.shape[0]-extra_slices, orig_data.shape[1], orig_data.shape[2])
        ...
        if self.does_inpainting:
            assert(lesion_mask is not None), "Inpainting requires a lesion mask, mismatch between chosen model and input data"

        pred_prob = torch.zeros(shape, **kwargs)
        loc_map = torch.zeros(shape_loc, **kwargs) if any(self.does_localisation.values()) else None
        ...
        for plane, model in self.models.items():
            ...
            if self.does_localisation[plane]:
                
                pred_prob, loc_map = model.run(init_pred=pred_prob, img_filename=orig_f, orig_data=orig_data, lesion_mask=lesion_mask, orig_zoom=zoom, out=pred_prob, sdir=f'{self.get_out_dir()}/{self.get_subject_name()}/', pad= not crop)

            elif self.does_inpainting:
                 pred_prob, inpainted_volume = pred_prob, None
            else:
                if not crop:
                    pred_prob = model.run(init_pred = pred_prob, img_filename=orig_f, orig_data=orig_data, lesion_mask=lesion_mask, orig_zoom=zoom, out=pred_prob, pad=True)
                else:
                    pred_prob = model.run(init_pred = pred_prob, img_filename=orig_f, orig_data=orig_data[extra_slices//2:orig_data.shape[0]-extra_slices//2,:,:], lesion_mask=lesion_mask, orig_zoom=zoom, out=pred_prob, pad=True)
        ...
        # get AC and PC from loc_map
        if any(self.does_localisation.values()):
            # remove last dimension
            loc_map = loc_map.cpu().squeeze()
            # calculate AC and PC from loc_map
            orig, orig_data = self.get_img(orig_f)
            self.save_img(os.path.join(self.out_dir, self.subject_name, 'mri', 'loc_map.mgz'), loc_map, orig, dtype=np.float32)

            ac_pc = calculate_centers_of_comissures(loc_map)

            LOGGER.debug(f"AC: {ac_pc[0]}, PC: {ac_pc[1]}")

        if self.does_inpainting:
            return pred_classes, inpainted_volume
        elif loc_map is not None:
            return pred_classes, ac_pc
        else:
            return pred_classes
...
def main():
    ...
    parser = parser_defaults.add_plane_flags(parser, "config",
                                             {"coronal": "CCNet/config/FastSurferVINN_coronal.yaml",
                                              "axial": "CCNet/config/FastSurferVINN_axial.yaml",
                                              "sagittal": "CCNet/config/FastSurferVINN_sagittal.yaml"})
    ...
    parser.add_argument('--inferece_weights', nargs=3, type=float, default=[0.6,0.2,0.2], help='Weights for the inference of the three planes')
    ...
    print("Checkpoints: " +args.ckpt_ax, args.ckpt_cor, args.ckpt_sag)
    ...
    for subject in s_dirs:
        ...
        original_size = orig_img.header.get_data_shape()
        # TODO: Align image to midplan 
        # We currently assume the image is already aligned to the midplane of 128. 
        # Assumes midplane ON sagittal voxel plane 128 (not in between)
        midplane = 128
        if args.crop:
            volume_thickness = np.ceil((5/orig_img.header.get_zooms()[0]))
            if volume_thickness % 2 == 0:
                volume_thickness += 1
            offset = midplane - volume_thickness//2
            extra_slices = eval.view_ops['sagittal']['cfg'].MODEL.NUM_CHANNELS//2*2
            volume_thickness += extra_slices
            data_array = du.subset_volume_plane(data_array, plane=midplane, thickness=volume_thickness, axis=0)
        else:
            # already cropped
        ...
        # Run model
        try:
            # orig_f: str, orig_data: np.ndarray, zoom: Union[np.ndarray, tuple]
            pred_data = eval.get_prediction(orig_f=orig_fn, orig_data=data_array, zoom=orig_img.header.get_zooms(), lesion_mask=lesion_mask, crop = args.crop, extra_slices=extra_slices)
            if eval.does_inpainting:
                pred_data, inpainted_volume = pred_data

                inpainted_volume = inpainted_volume.cpu().numpy()
                inpainted_volume = np.round(inpainted_volume * 255).astype(np.uint8)
                
                # Save inpainted volume
                eval.save_img(os.path.join(eval.out_dir, eval.subject_name, 'mri', 'inpainted_network_output.mgz'), inpainted_volume, orig_img, dtype=np.uint8)

                inpainted_volume[~lesion_mask] = data_array[~lesion_mask]
                eval.save_img(os.path.join(eval.out_dir, eval.subject_name, 'mri', 'inpainted.mgz'), inpainted_volume, orig_img, dtype=np.uint8)
            elif np.array([value for value in eval.does_localisation.values()]).any():
                pred_data, ac_pc = pred_data
                ...
                points_file = os.path.join(eval.out_dir, eval.subject_name, 'mri', 'points.txt')
                if not os.path.exists(points_file):
                    os.makedirs(os.path.dirname(points_file), exist_ok=True)
                with open(points_file, 'w') as f:
                    f.write(f'Type,r,a,s\n')
                    f.write(f'PC,{pc[0]},{pc[1]},{pc[2]}\n')
                    f.write(f'AC,{ac[0]},{ac[1]},{ac[2]}')


            # todo: pad to original size if crop
                    
            if args.crop:
                pred_data = du.pad_to_size(pred_data, original_size, plane = 128, axis = 0)
