...
This directory contains all information needed to run inference with the readily trained FastSurferVINN or train it from scratch. CCNet is capable of whole brain segmentation into 95 classes in under 1 minute, mimicking FreeSurfer's anatomical segmentation and cortical parcellation (DKTatlas). The network architecture incorporates local and global competition via competitive dense blocks and competitive skip pathways, as well as multi-slice information aggregation that specifically tailor network performance towards accurate segmentation of both cortical and sub-cortical structures. 
...
The *CCNet* directory contains all the source code and modules needed to run the scripts. A list of python libraries used within the code can be found in __requirements.txt__. The main script is called __run_prediction.py__ within which certain options can be selected and set via the command line:
...
To run the network on MRI-volumes of subjectX in ./data (specified by `--t1` flag; e.g. ./data/subjectX/t1-weighted.nii.gz), change into the *CCNet* directory and run the following commands: 
...
To run the network on all subjects MRI-volumes in ./data, change into the *CCNet* directory and run the following command: 
...
The *CCNet* directory contains all the source code and modules needed to run the scripts. A list of python libraries used within the code can be found in __requirements.txt__. The main training script is called __run_model.py__ whose options can be set through a configuration file and command line arguments:
...
#### Model options
* MODEL_NAME: Name of model [CCNet, FastSurferVINN]. Default: FastSurferVINN
...
To train the network on a given hdf5-set, change into the *CCNet* directory and run
...
### Example Command: Training CCNet
Trains CCNet using a provided configuration file and specifying no augmentations:
```
python3 run_model.py \
--cfg custom_configs/CCNet.yaml \
--aug None
```
