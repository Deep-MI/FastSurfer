...
h_cortex = (
    "optional path to hemi.cortex for optional masking of annotation to only cortex"
)
h_outannot = "path to output annotation file"
h_append = (
    'optional, e.g. ".thresh" can be appended to label names (I/O) for exvivo FS labels'
)
...
def options_parse():
    ...
    if (
        options.hemi is None
        or options.colortab is None
        or options.labeldir is None
        or options.white is None
    ...
    if (
        ...
        or options.srcsphere is not None
        or options.trgdir is not None
