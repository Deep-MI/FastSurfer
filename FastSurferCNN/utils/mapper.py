...
from typing import Callable, TypeVar, Generic, Mapping, Tuple, Optional, Union, Set, overload, cast, Hashable, Dict, \
    Sequence, List, Iterable, Any, TextIO, Literal, Container, Collection, Iterator
...
class Mapper(Generic[KT, VT]):
    ...
    def __init__(self, mappings: Mapping[KT, Union[VT, npt.NDArray[VT]]], name: str = "undefined"):
        """
        ...
        if all(isinstance(k, Number) for k in keys):
            ...
            if extreme_labels[0] < 0:
                raise NotImplementedError(f"Negative classes are not supported (min: {extreme_labels[0]}, "
            ...
            raise TypeError("Invalid type for keys of the mapping, must either be all numeric or all strings.")
    ...
    def update(self, other: 'Mapper[KT, VT]', overwrite: bool = True) -> 'Mapper[KT, VT]':
    ...
    def map(self, image: AT, out: Optional[AT] = None) -> AT:
        ...
        if isinstance(out_type, np.ndarray):
            if not hasattr(self, "_map_np"):
                if self._max_label > 4096:
                    ...
                    lil = sparse.lil_array if hasattr(sparse, "lil_array") else sparse.lil_matrix
            ...
            if not hasattr(self, "_map_torch"):
                self._map_torch = out_type.new_full((self._max_label + 1,) + self._label_shape, fill_value=0)
            else:
                init_map = False
        ...
        try:
            ...
            map_unique = np.unique(_map) if isinstance(_map, np.ndarray) else _map.unique()
        ...
        if torch.is_tensor(mapped) and (hasattr(mapped, 'is_sparse') and mapped.is_sparse):
            mapped.to_dense()
        try:
    ...
    def _map_py(self, image: AT, out: Optional[AT] = None) -> AT:
        ...
        if out is None:
            def _internal_map(img):
                return [self._map_dict[v] if img.ndim == 1 else _internal_map(v) for v in img]

            return to_same_type(_internal_map(image), type_hint=out_type)
        else:
            def _internal_map(img, o):
                if img.ndim == 1:
                    o[:] = to_same_type([self._map_dict[v] for v in image], type_hint=out_type)
                else:
    ...
    def __call__(self,
                 image: AT,
                 label_image: Union[npt.NDArray[KT], torch.Tensor]) -> Tuple[AT, Union[npt.NDArray, torch.Tensor]]:
        """
    ...
    def chain(self, other_mapper: 'Mapper[VT, T_OtherValue]') -> 'Mapper[KT, T_OtherValue]':
        """
        ...
        if any(is_target_set):
            ...
            raise ValueError(f"The target space must be hashable, but {is_target_set.count(True)} values are not "
        ...
        if not target_space <= other_mapper.source_space:
            ...
            raise ValueError(f"The first set ({self.name}) maps to the following keys, that the second mapper "
                             f"({other_mapper.name}) does not map from:\n  " +
                             ", ".join(f"'{v}'" for v in target_space - other_mapper.source_space))
        return Mapper(dict((in_key, other_mapper[out_key]) for in_key, out_key in self._map_dict.items()),
                      name=f"{self.name} -> {other_mapper.name}")

    @classmethod
    def make_classmapper(cls, mappings: Dict[int, int],
                         keep_labels: Sequence[int] = tuple(),
                         compress_out_space: bool = False,
                         name: str = "undefined") -> 'Mapper[int, int]':
        ...
        if compress_out_space:
            target_labels = dict((v, i) for i, v in enumerate(sorted(set(mappings.values()))))
    ...
    def _map_logits(self, logits: AT, axis: int = -1, reverse: bool = False, out: Optional[AT] = None,
                    mode: Literal['logit', 'prob'] = 'logit') -> AT:
        ...
        if mode == 'logit':
            reduce_func = np.multiply.reduce if isinstance(logits, np.ndarray) else partial(reduce, torch.mul)
            spread_func = np.float_power if isinstance(logits, np.ndarray) else torch.pow
        elif mode == 'prob':
            reduce_func = np.add.reduce if isinstance(logits, np.ndarray) else partial(reduce, torch.add)
            spread_func = np.multiply if isinstance(logits, np.ndarray) else torch.mul
        else:
            raise ValueError(f"Unknown mode, should be in 'logit', 'prob', but was '{mode}'.")

        from collections import defaultdict
        data = defaultdict(list)
        mappings = self._map_dict.items()
        if reverse:
            unique_target_classes = np.unique(list(self._map_dict.values()), return_counts=True)
        ...
        for source_dim, target_dim in mappings:
            ...
            if cts > 1:
                this_logit = spread_func(this_logit, 1. / cts)
        ...
        if not np.all(labels == np.arange(labels.size)):
            shape[axis] = 1
            alloc = partial(np.zeros, like=logits) if isinstance(logits, np.ndarray) else partial(logits.new_full, fill_value=0)
            zeros = alloc(shape)
        else:
            zeros = None

        data = {k: v[0] if len(v) == 1 else reduce_func(v) for k, v in data.items()}
        return stack([data[i] if i in labels else zeros for i in range(labels.max(initial=-1)+1)], axis, out=out)
...
class ColorLookupTable(Generic[KT]):
    ...
    def __init__(self,
        ...
        if isinstance(colormap, Colormap) or callable(colormap) or isinstance(colormap, str):
    ...
    def color_palette(self, color_palette: Union[Dict[KT, npt.ArrayLike], npt.ArrayLike, None]):
        ...
        if color_palette is None:
            ...
            if isinstance(color_palette, dict):
                if self._classes is None:
                    raise RuntimeError("No classes defined, but setting a color_palette via dict.")
    ...
    def getitem_by_index(self, index: int) -> Tuple[int, KT, Tuple[int, int, int, int], Any]:
        """Returns index, key, colors and additional values for the key."""
        color = self.get_color_by_index(index, 255)
        return index, self._classes[index], color, None

    def get_color_by_index(self, index: int, base: NT = 1.) -> Tuple[NT, NT, NT, NT]:
        ...
        if len(_color) > 3:
            ...
            raise RuntimeError(f"Invalid shape of the color_palette, only {len(_color)} elements.")
        return color

    def colormap(self) -> Mapper[KT, ColorTuple]:
        if self._color_palette is None:
            raise RuntimeError("No color_palette set")
        return Mapper(dict(zip(self.classes, self.color_palette)), name="color-" + self.name)
    ...
    def labelname2index(self) -> Mapper[KT, int]:
        ...
        return Mapper(dict(zip(self._classes, range(len(self._classes)))), name="index-" + self.name)
...
class JsonColorLookupTable(ColorLookupTable[KT]):
    ...
    def __init__(self, file_or_buffer,
        ...
        if isinstance(file_or_buffer, str) and file_or_buffer.lstrip().startswith("{"):
            ...
            raise TypeError(f"file_or_buffer should be either a file pointer or a string, but was a "
        ...
        if len(set(classes)) != len(classes):
            ...
            raise KeyError(f"Duplicate classes in source_space: {unique_classes[counts>1]}")
        super(JsonColorLookupTable, self).__init__(classes=classes, color_palette=color_palette, colormap=colormap,
                                                   name=name)

    def _get_labels(self) -> Union[Dict[KT, Any], Iterable[KT]]:
        return self._data["labels"] if isinstance(self._data, dict) and "labels" in self._data else self._data
    ...
    def labelname2id(self) -> Mapper[KT, Any]:
        ...
        return Mapper(dict(zip(self._classes, labels.values())), name="value-" + self.name)
...
class TSVLookupTable(ColorLookupTable[str]):
    ...
    def __init__(self, file_or_buffer, name: Optional[str] = None, header: bool = False,
                 add_background: bool = True) -> None:
        """
        ...
        self._data = pandas.read_csv(file_or_buffer, delim_whitespace=True, index_col=0, skip_blank_lines=True,
                                     comment="#", header=int(header), names=names.keys(), dtype=names)
        if not (self._data.index == 0).any():
            df = pandas.DataFrame.from_dict({k: [v] for k, v in zip(names.keys(), [0, "Unknown", 0, 0, 0, 0])})
            self._data = pandas.concat([df, self._data])
        classes = self._data["Label name"].tolist()
        channels = ["Red", "Green", "Blue", "Alpha"]
        color_palette = np.asarray([tuple(int(row[k].item()) for k in channels) for row in self._data.iloc])
        super(TSVLookupTable, self).__init__(classes=classes, color_palette=color_palette,
                                             name=name)

    def getitem_by_index(self, index: int) -> Tuple[int, str, Tuple[int, int, int, int], int]:
        """
    ...
    def labelname2id(self) -> Mapper[KT, Any]:
        ...
        Raises:
            RuntimeError, if no value is associated.
        """
        return Mapper(dict(zip(self._classes, self._data.index)),
