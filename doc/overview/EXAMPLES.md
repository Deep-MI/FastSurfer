...
* The `--user $(id -u):$(id -g)` part automatically runs the container with your group- (id -g) and user-id (id -u). All generated files will then belong to the specified user. Without the flag, the docker container will be run as root which is discouraged. However, if you use Docker in the **rootless mode** with your group- and user-id, FastSufer does not have the write permission for the output directory. For the rootless mode, you should set `--user 0` and add the `--allow_root` flag. There will be no write permission issue for the output directory and you will be the owner of files created by FastSurfer.
