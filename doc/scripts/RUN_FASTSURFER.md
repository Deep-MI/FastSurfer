...
* `--lesion_mask`: Path to a binary lesion mask (in the same space as the T1 input). If provided, FastSurfer will perform lesion inpainting using LIT before running the segmentation and surface pipelines. This is useful for images with tumors or other large lesions.
