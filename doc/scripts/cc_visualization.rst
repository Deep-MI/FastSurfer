...
.. note::
   ...
   You can still pass ``--contours``, ``--thickness`` and
   ``--measurement_points`` directly when working with standalone files, but
   ``--template_dir`` is the recommended way to load the multi-slice templates
