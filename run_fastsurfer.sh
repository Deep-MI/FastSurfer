...
case $key in
  ...
  --seg|--asegdkt_segfile|--aparc_aseg_segfile)
    ...
    shift # past value
    ;;
  # several simple options that *just* set a flag
  ...
  # several options that assign a flag, but have additional code
  ...
  *)    # unknown option
    ...
    if [[ "$1" != "" ]] ; then echo "ERROR: Flag '$1' unrecognized." ;  exit 1 ; fi
    ;;
esac
done
